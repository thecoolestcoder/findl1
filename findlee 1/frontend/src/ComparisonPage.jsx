...
// --- 1. INLINED HOOK: Dark Mode (Fixed import error) ---
const useDarkMode = () => {
...
const apiKey = "AIzaSyAQfpcaNXbkrYO2gpi1F2FpnS4FHZM2mOo"; // The execution environment provides the key at runtime
const API_URL = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`;
const THROTTLE_DELAY_MS = 2000;

// Helper: Centralized API Call with Throttling and Retry
...
// --- ORIGINAL PARSING LOGIC (Preserved) ---
// --- ORIGINAL PARSING LOGIC (Preserved) ---
// --- ORIGINAL PARSING LOGIC (REFINED FOR AESTHETICS) ---
const parseComparisonResult = (text, isDarkMode) => {
  ...
  // Note: HeadingColor is no longer used for text inside the loop.
  ...
  // 🛠️ REFACTORED FUNCTION FOR ROBUST HEADING PARSING AND AESTHETIC ICON LOGIC
  const formatProductContent = (content, isDarkMode) => {
    if (!content) return '';
    
    // 1. Convert markdown bold to HTML strong tags first
    let formattedContent = content.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>');
    let html = '';
    
    // 2. Robustly split the content by any bold heading (The resilient method)
    const innerSplitRegex = /(<strong>.*?<\/strong>)/g;
    const innerParts = formattedContent.split(innerSplitRegex).filter(part => part.trim() !== '');

    let processedBlocks = [];

    // 3. Reconstruct the blocks, pairing headings with their content
    for (let i = 0; i < innerParts.length; i++) {
        let part = innerParts[i].trim();
        
        // Check if it's a section heading (starts/ends with strong, contains colon)
        if (part.startsWith('<strong>') && part.endsWith('</strong>') && part.includes(':')) {
            // Extract clean heading name
    ...
    // 4. Iterate over the correctly separated blocks and render the HTML
    processedBlocks.forEach(block => {
        // Aesthetic color vars
        ...
        if (block.heading) {
            // --- SECTION LOGIC (Pros, Cons, Features) ---
            ...
            // Set up colors for the new PILL BADGE structure
            ...
            // NEW CLEANER LIST STRUCTURE: No heavy list box background, use left border for color
            if (contentBody.includes('\n') || contentBody.includes('*') || contentBody.includes('-') || contentBody.includes('•')) {
                html += `<ul class="space-y-3 list-none pl-0 pt-2">`; // Reduced padding
                
                contentBody.split(/[\n*\-•]/)
                   .map(item => item.trim())
                   .filter(item => item.length > 0)
                   .forEach(item => {
                       // Item: Left border + slightly raised background for dark mode
...
// --- COMPONENT: StageLoader (Preserved) ---
...
// --- COMPONENT: ResultModal (Preserved) ---
// --- COMPONENT: ResultModal (AESTHETICALLY REFINED) ---
const ResultModal = ({ result, onClose, isMainDarkMode }) => {
    ...
    const productCardBg = isDarkMode ? 'bg-gray-800/70 border-gray-700' : 'bg-gray-50 border-gray-200'; // Slightly lighter background for dark mode
    // NEW GRADIENT TITLE BAR
...
const ComparisonPage = () => {
    // 🛑 ADDED HELPER FUNCTION: To read the mode from the URL
    const getUrlParameter = (name) => {
        name = name.replace(/[\[]/, '\\[').replace(/[\]]/, '\\]');
        const regex = new RegExp('[\\?&]' + name + '=([^&#]*)');
        // Using window.location.search to get the query string
        const results = regex.exec(window.location.search);
        return results === null ? '' : decodeURIComponent(results[1].replace(/\+/g, ' '));
    };

    // 🛑 UPDATED INITIAL STATE: Check URL for 'mode=select'
    const initialMode = getUrlParameter('mode') === 'select' ? 'select' : 'prompt';
    const [comparisonMode, setComparisonMode] = useState(initialMode);
    
    // [OLD CODE was: const [comparisonMode, setComparisonMode] = useState('prompt'); ]
    ...
    // 3. ADDED: Request Timestamp Ref for Throttling
    const lastRequestTime = useRef(0);

    const { isDarkMode, toggleDarkMode } = useDarkMode(); // Removed unused variables themeMode, useSystemPreference
    ...
    const scrollToCompare = () => {
        ...
        if (compareSection) {
            // Get the element's position from the top of the document
            const targetPosition = compareSection.offsetTop;
            
            // Define an extra scroll offset (e.g., 150 pixels down)
            const scrollOffset = 150; 
            
            // Use window.scrollTo for precise, offset-based scrolling
            window.scrollTo({
                top: targetPosition + scrollOffset, // Scroll past the element's top position by the offset
                behavior: "smooth",
            });
        }
    };

    // NEW EFFECT ADDED FOR AUTO-SCROLL
    useEffect(() => {
        // Only auto-scroll if the mode is 'select' on initial load
        if (comparisonMode === 'select') {
            // Use a slight delay to ensure the DOM is fully laid out before scrolling
    ...
    // 4. UPDATED HANDLER: Uses unified helper
        ...
        * Point 1
        ...
        try {
            ...
            if (result) {
                ...
                setLoading(false); // Stop loading if throttled or error
            }
        } catch (error) {
            setLoading(false);
    ...
    // 5. UPDATED HANDLER: Uses unified helper
